...
extension CachedMovie {
    ...
    static func from(dto: MovieDTO, context: NSManagedObjectContext) -> CachedMovie {
    ...
    static func toDTOArray(_ movies: [CachedMovie]) -> [MovieDTO] {
        return movies.map { $0.toDTO() }
    }

    /// Converts array of MovieDTO to array of CachedMovie
    static func fromDTOArray(_ dtos: [MovieDTO], context: NSManagedObjectContext) -> [CachedMovie] {
